...
import { CalendarDays, CheckCircle2, Coins, Radio, TrendingUp, type LucideIcon } from 'lucide-react';
import {
    ...
    <>
      ...
      <div className="mt-5 grid min-w-0 gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <ProgressMetricCard
          ...
          valueClassName={progressDelta >= 0 ? 'text-emerald-600' : 'text-red-500'}
...
function BaselineCard({
  ...
  return (
    <section className="glass-panel flex min-h-48 w-full max-w-[calc(100vw-3rem)] min-w-0 flex-col p-5 sm:max-w-full">
      <div className="flex items-start gap-4">
        <span className="group relative">
          <button
            className="grid h-16 w-16 place-items-center rounded-full border border-emerald-300/50 bg-emerald-500/12 text-emerald-600 shadow-inner shadow-white/50 transition hover:bg-emerald-500/18"
          ...
          <span
            ...
            role="tooltip"
            className={hoverTooltipClasses('left-0 top-20 w-max whitespace-nowrap px-3 py-2')}
          >
            Start tracking
          </span>
        </span>
      ...
      {baseline && (
        <div className="mt-5 flex w-fit items-center gap-2 rounded-full border border-emerald-300/50 bg-emerald-500/12 px-3 py-1 text-sm font-bold text-emerald-700">
          <CheckCircle2 className="h-4 w-4" />
          Baseline set
        </div>
      )}
      <p className="mt-auto max-w-xs wrap-break-word pt-5 text-sm leading-6 text-slate-700">
...
function ProgressMetricCard({
  ...
  return (
    <section className="glass-panel flex min-h-48 w-full max-w-[calc(100vw-3rem)] min-w-0 flex-col p-5 sm:max-w-full">
      ...
      <p className={`mt-6 wrap-break-word text-center text-2xl font-bold tracking-normal ${valueClassName}`}>
        <MetricValue>{value}</MetricValue>
      </p>
      <p className="mt-auto pt-4 text-center text-sm font-bold text-slate-500">{helper}</p>
    </section>
  );
}
